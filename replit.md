...
10. **Dynamic Topic List** - Subject/topic dropdown in user topic selection is now dynamically populated from the database instead of a hardcoded list. When admins generate questions for new subjects (like "Medical Surgical"), they automatically appear in the user's dropdown without code changes.
11. **CRITICAL FIX: Subject Column in Generation Jobs** - Added dedicated `subject` column to `generationJobs` table to properly separate the main subject area (e.g., "English", "Math") from specific topics (e.g., "Sentence Structure"). This prevents subject mixing during equal distribution where topics were being used as subject context, causing Math content to appear in English questions. Admin form now uses separate "Subject Area" field.
